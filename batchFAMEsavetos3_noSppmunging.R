...
try({
  ...
  for (myFH in rawFHPaths)
    try({
      ...
      if (rv$usePUpolys == TRUE) {
        rv$outputFH <-
          paste(rv$outputFH, file_path_sans_ext(basename(rv$puName)), sep = "_")
      }
      ...
      #log.file
      my_log <-
        file(file.path(rv$resultsDir, "my_log.txt")) # File name of output log
      ...
      FHanalysisPath <-
        file.path(resultsDir , paste0(gsub(".shp", "", rv$outputFH), "FHanalysis", ".qs"))
      ...
      checkSaved <-
        system(paste0(
          "aws s3 ls ",
          " s3://fame-obm/results/",
          basename(FHanalysisPath)
        ))
      if (checkSaved == 0) {
        file.remove(FHanalysisPath)
      }
      ...
      try({
        ...
        rv$GS_Summary_PU_LONG <-
          rv$GS_Summary$GS_Summary_wide %>% dplyr::select(-c(
            MIN_LO_TFI,
            MIN_HI_TFI,
            MAX_TFI,
            Index,
      ...
      if (doSpeciesCalculations) {
        try({
          ...
          system("aws s3 sync ./results s3://fame-obm/results")
          checkSaved <-
            system(paste0(
              "aws s3 ls ",
              " s3://fame-obm/results/",
              basename(analysisPath)
            ))
          
          if (checkSaved == 0) {
            file.remove(analysisPath)
          }
        })
      }
      ...
      checkSaved <-
        system(paste0(
          "aws s3 ls ",
          " s3://fame-obm/results/",
          basename(rv$resultsDir)
        ))
      if (checkSaved == 0) {
        unlink(rv$resultsDir, recursive = TRUE)
      }

...
for(i in 1:nrow(myqs1)){
  ...
  #This next section is a cludge to get around 2 billion row problem for pivot longer here at the moment splits it into two - potentially coudl do more and then parralellise
  ...
  #   }
  # 
  #ResultsMC<-mclapply(2:N,bigMunge)
...
# NR<-nrow(mySppFiles)
# #fixing up missing values that did not write
# mySppFiles$AddedRepandPercent<-0
# for(i in 1:NR){
#   mytab<-fread(mySppFiles$SppSumFile[i])
#   mytab$AbundType<-mySppFiles$AbundType[i]
#   mytab$Replicate<-mySppFiles$Replicate[i]
#   mytab$Percent<-mySppFiles$Percent[i]
#   mySppFiles$AddedRepandPercent[i]<-1
#   write_csv(mytab,mySppFiles$SppSumFile[i])
#   print(paste(i ,"of",NR))
# }
# write_csv(mySppFiles,"Finishedand addedbackValues.csv")  
#mySppFiles <- read_csv("Finished and addedbackValues.csv")%>% filter(SppFileExists==TRUE)
